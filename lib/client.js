...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const APPROVE_TEXTS = ["批准", "同意", "允许", "确认执行", "approve", "allow", "accept", "confirm"];
		...
		function approvalContext() {
			for (const card of document.querySelectorAll('[class*="Mbwy4a_card"]')) {
				if (card.className.includes("cardMinimized")) continue;
		...
		function actToggleSidebar() {
			...
			for (const btn of document.querySelectorAll('[data-pane="sidebar"] [class*="iconButton"]')) {
		...
		function actNewWorkspace() {
			...
			for (const btn of [...document.querySelectorAll('[data-pane="sidebar"] button'), ...document.querySelectorAll('[class*="workspace"] button')]) {
				if (!isVisible(btn)) continue;
		...
		function actNewSession() {
			...
			for (const btn of document.querySelectorAll('[data-pane="sidebar"] button')) {
				if (isVisible(btn)) {
					const name = accessibleName(btn);
		...
		function findComposerInput() {
			const candidates = Array.from(document.querySelectorAll('textarea, [contenteditable="true"], [contenteditable=""]'))
				.filter(isVisible)
				.filter((el) => !(el.closest && (el.closest("[data-dsh-panel-host]") || el.closest('[class*="Mbwy4a"]'))));
		...
		function refreshAvailability() {
			...
			const sidebarToggle = hasVisible('[data-pane="sidebar"] [class*="iconButton"]');
			...
			const newSession = hasVisible('[data-pane="sidebar"] button');
			state.availability = {
				"toggle.sidebar": layout || sidebarToggle,
				"toggle.details": layout || sidebarPanel,
				"new.workspace": hasVisible('[data-pane="sidebar"] button') || hasVisible('[class*="workspace"] button'),
				...
				get: () => ({
					...
					readiness: () => {
						...
						return {
							...
							"new.workspace: button": hasVisible('[data-pane="sidebar"] button') || hasVisible('[class*="workspace"] button'),
